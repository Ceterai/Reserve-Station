...
using Content.Shared._Reserve.Overlays;
...
public sealed class MoodSystem : EntitySystem
{
    ...
    private void OnRoleAdded(RoleAddedEvent args)
    {
        ...
        if (_roleSystem.MindHasRole<TraitorRoleComponent>(args.MindId))
            RaiseLocalEvent(ownedEntity, new MoodEffectEvent("TraitorFocused"));

        if (_roleSystem.MindHasRole<RevolutionaryRoleComponent>(args.MindId))
            RaiseLocalEvent(ownedEntity, new MoodEffectEvent("RevolutionFocused"));

        if (_roleSystem.MindHasRole<CosmicCultRoleComponent>(args.MindId))
